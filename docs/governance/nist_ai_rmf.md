...
| **MAP** | MAP 5.1 — impacts (likelihood/magnitude) of system behavior are characterized | `results/faults.csv` (336-cell fault campaign), `configs/default.yaml` (`faults.campaign` grid: 6 components x 4 fault types x 7 patterns x 2 platforms) | The fault campaign systematically enumerates failure modes per component and classifies each cell FOUR ways — PROPAGATED (failures spread beyond the faulted component: P1/P4 SPOF, P2 chain), ISOLATED (only traversing requests affected: P5 bulkhead, P7 remote cluster), ABSORBED (requests still succeed at elevated latency: P3 buffered redelivery, P6 deferred human review), or NOT_EXERCISED (structurally irrelevant component) — an executed impact map matching the paper's Table 3 consequences (Phase 2 task 104 recalibration). |
...
| **MEASURE** | MEASURE 2.6 — system is evaluated in conditions similar to deployment, including failure modes | `figures/fault_matrix.png` (PROPAGATED / ISOLATED / ABSORBED / NOT_EXERCISED grid per platform; OUTAGE faults use cross-request outage windows), `results/faults.csv` (per-cell classification, traversing/non-traversing success rates, baseline-vs-fault latency), `src/agentorch/rig/faultcampaign.py` (Algorithm 2) | Fault-mode behavior is exercised and visualized per (pattern, component, fault type) cell rather than assumed; every exercised fault visibly degrades its condition (asserted in `tests/test_paper_agreement.py`). |
| **MEASURE** | MEASURE 2.9 / 4.2 — results of measurement are documented with statistical rigor | `docs/STATISTICAL_ANALYSIS_PLAN.md`, `src/agentorch/stats/bootstrap.py` (BCa CIs), `src/agentorch/stats/compare.py` (Mann-Whitney U, rank-biserial, Hodges-Lehmann), `src/agentorch/stats/correction.py` (Holm), `figures/table4_supplementary.csv` (grades derived under that plan) | The measurement pipeline is pre-committed and multiple-comparison-corrected; effect sizes accompany p-values. |
...
|---|---|---|
| Human-AI configuration: define when GAI output requires human review | Confidence gate + threshold (`HitlGovernor.confidence_threshold`, `governance/hitl_example.py`); P6 routing fraction in S3 (`configs/default.yaml`); configurable human decision delay (`latency.shared.human_decision_delay`) that dominates P6 end-to-end latency in the recalibrated study (`tests/test_paper_agreement.py` claim 2) | Threshold and human decision time are explicit, tunable governance parameters; the measured latency price of oversight is quantified, not hidden. |
